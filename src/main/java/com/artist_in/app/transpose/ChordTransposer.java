...
/**
 * Pure chord-math utility. No Spring, no DB, no session state — just musical
 * note shifting. Fully unit-testable in isolation.
 *
 * If a transpose bug ever shows up in the actual chord/key values (wrong note,
 * wrong enharmonic spelling, etc.) — this is the ONLY file to check. If the bug
 * is about "which offset gets applied" or "state not syncing" — that's
 * TransposeServiceImpl or the Flutter client, not this file.
 */
