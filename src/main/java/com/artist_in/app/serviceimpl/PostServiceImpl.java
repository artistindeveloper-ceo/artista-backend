...
public class PostServiceImpl implements PostService {
    ...
    @Cacheable(
            value = "post",
            key = "#targetUserId + ':' + (#viewerId != null ? #viewerId : 'guest') + ':' + #pageable.pageNumber + ':' + #pageable.pageSize",
            unless = "#result == null"
    )
    ...
    @Cacheable(
            value = "feed",
            key = "#viewerId + ':' + #pageable.pageNumber + ':' + #pageable.pageSize"
    )
    ...
    private PostResponse toResponse(Post post, User viewer) {
        boolean likedByViewer = viewer != null
        ...
        return PostResponse.builder().id(post.getId()).author(UserMapper.toSummary(post.getAuthor()))
                ...
                .likedByViewer(likedByViewer).viewsCount(post.getViewsCount())
