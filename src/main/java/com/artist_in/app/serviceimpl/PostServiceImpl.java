...
public class PostServiceImpl implements PostService {
	...
	public PostResponse createPost(Long authorId, CreatePostRequest request) {
		...
		boolean isVideo = request.getMediaType() == MediaType.VIDEO;
		String mediaUrl = null;
		PostStatus status = PostStatus.READY;

		if (request.getMediaKey() != null) {
			if (isVideo) {
				status = PostStatus.PROCESSING; // video abhi raw hai, backend process karega
			} else {
				mediaUrl = mediaService.cdnUrl(request.getMediaKey()); // image turant ready
			}
		}

		Post post = Post.builder().author(author).caption(request.getCaption()).mediaUrl(mediaUrl)
				.mediaType(request.getMediaType() == null ? MediaType.NONE : request.getMediaType()).status(status)
				.build();

		post = postRepository.save(post);
		log.info("Post created: id={}, authorId={}, status={}", post.getId(), authorId, status);

		if (isVideo) {
			videoProcessingService.processAsync(post.getId(), request.getMediaKey());
		}
	...
	@Override
	@Transactional(readOnly = true)
	@Cacheable(value = "post", key = "#targetUserId + ':' + (#viewerId != null ? #viewerId : 'guest') + ':' + #pageable.pageNumber + ':' + #pageable.pageSize", unless = "#result == null")
	public PageResponse<PostResponse> getUserPosts(Long targetUserId, Long viewerId, Pageable pageable) {
		...
		User viewer = viewerId != null ? userService.getUserOrThrow(viewerId) : null;

		Page<Post> page = postRepository.findByAuthorAndIsArchivedFalseOrderByCreatedAtDesc(target, pageable);
		return PageResponse.from(page, post -> toResponse(post, viewer));
	}
	...
	@Override
	@Transactional(readOnly = true)
	@Cacheable(value = "feed", key = "#viewerId + ':' + #pageable.pageNumber + ':' + #pageable.pageSize")
	public PageResponse<PostResponse> getFeed(Long viewerId, Pageable pageable) {
		...
		// Viewer pehle se hi ek baar fetch ho raha tha - iska object ab toResponse()
		// ko diya jayega taaki wahan dobara DB call na ho (N+1 fix)
	...
	public PageResponse<PostResponse> getExploreFeed(Long viewerId, Pageable pageable) {
		...
		// N+1 FIX: already fetched 'viewer' object reuse karo, dobara fetch nahi
		return PageResponse.from(page, post -> toResponse(post, viewer));
	}
	...
	// Single-post call sites (getPost, createPost) ke liye - yahan loop nahi hai,
	// isliye ek extra query se N+1 nahi banta. Delegate karta hai asli overload ko.
	...
	private PostResponse toResponse(Post post, User viewer) {
		boolean likedByViewer = viewer != null && postLikeRepository.existsByPostAndUser(post, viewer);
		...
		return PostResponse.builder().id(post.getId()).author(UserMapper.toSummary(post.getAuthor()))
				...
				.likedByViewer(likedByViewer).viewsCount(post.getViewsCount()).createdAt(post.getCreatedAt()).build();
	}
